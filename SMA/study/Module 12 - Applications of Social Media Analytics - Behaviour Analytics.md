...
## 12.3 Anomaly Detection in Social Networks

- **Anomaly detection** aims to identify unusual or deviations from expected patterns of behaviors within the network.
- As networks grow, detecting structurally abnormal nodes, edges, or subgraphs is critical for security and platform health.

### Types of Anomalies in Social Media
- **Node Anomalies**: Individual accounts that behave distinctly differently from the rest (e.g., extremely high degree with very low interactions, isolated spam bursts).
- **Edge Anomalies**: Unexpected connections between disconnected communities, potentially indicating astroturfing or a compromised account.
- **Subgraph Anomalies**: Densely connected group formations happening in a very short time window (e.g., bot farms, link farming).

### Methods for Detection
- **Statistical Methods**: Using Z-scores on node degrees or posting frequencies to flag outliers.
- **Graph-Based Methods**: 
  - Subgraph analysis (identifying unnatural cliques)
  - Random walks (anomalous nodes often have unusual steady-state probabilities)
- **Machine Learning**: 
  - Using isolation forests or autoencoders to find data points that are difficult to reconstruct based on standard user patterns.
  - Analyzing temporal graphs to find sudden bursts or shifts in communication dynamics.
